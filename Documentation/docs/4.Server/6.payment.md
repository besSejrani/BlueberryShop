...
The PaymentIntent API is limited in scope, product informations can't be processed. It is still possible to store some custom ID in the metadata field, from there on, via a webhook, it is possible to retrieve all the informations.
